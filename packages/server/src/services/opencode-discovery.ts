...
export class OpenCodeDiscovery {
    ...
    if (agentsWithUrl.length > 0) {
      ...
      await Promise.allSettled(
        [...cwdGroups.values()].map(async (group) => {
          ...
          // Auto-rename agents with generic names to their session title
          for (const a of group) {
  ...
  async track(serverUrl: string, name?: string): Promise<{ agentId: string }> {
    ...
    // Use session title as agent name if no explicit name was provided
    const agentName = name || assignedSession?.title || `OpenCode@${new URL(serverUrl).port}`;
